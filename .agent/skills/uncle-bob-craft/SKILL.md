...
description: 'ALWAYS use this when the request matches Uncle BOB Craft: Use when performing code review, writing or refactoring code, or discussing architecture; complements clean-code and does not replace project linter/formatter.'
---
...
| **Clean Architecture** | Dependency Rule, layers, boundaries, SOLID in architecture | See [reference.md](references/legacy-reference.md) and [references/clean-architecture.md](./references/clean-architecture.md). |
| **The Clean Coder** | Professionalism, estimation, saying no, sustainable pace | See [reference.md](references/legacy-reference.md) and [references/clean-coder.md](./references/clean-coder.md). |
| **Clean Agile** | Values, Iron Cross, TDD, refactoring, pair programming | See [reference.md](references/legacy-reference.md) and [references/clean-agile.md](./references/clean-agile.md). |
| **Design patterns** | When to use, misuse, cargo cult | See [reference.md](references/legacy-reference.md) and [references/design-patterns.md](./references/design-patterns.md). |
...
Full lists (including heuristics C1–T9-style) are in [reference.md](references/legacy-reference.md). Use these in review to name issues and suggest refactors (extract, move dependency, introduce boundary).
...
- **Summaries, not the books.** Full Clean Code heuristics, component principles (REP/CCP/CRP, ADP/SDP/SAP), and detailed stories are in the books; we reference the most used parts. See [reference.md](references/legacy-reference.md) "Scope and attribution."
